function [Xt, D, Bt, params, stats] = OnAIR(Yt,lambda,mu,gamma,varargin)
%
% Syntax:       [Xt, D, Bt, params] = OnAIR(Yt,lambda,mu,gamma);
%               [Xt, D, Bt, params] = OnAIR(Yt,lambda,mu,gamma,opts);
%               [Xt, D, Bt, params, stats] = OnAIR(Yt,lambda,mu,gamma);
%               [Xt, D, Bt, params, stats] = OnAIR(Yt,lambda,mu,gamma,opts)
%               
...
%               
% Description:  Solves one of the following OnAIR (online adaptive image
%               reconstrution) problems:
%               
%               When type == 'hard':
%               
...
%               brimoor@umich.edu
%               
% References:   B. E. Moore, S. Ravishankar, R. R. Nadakuditi, and J. A.
%               Fessler, "Online data-driven image reconstruction using
%               efficiently learned dictionaries," IEEE Transactions on
%               Computational Imaging, 2018, to appear.
%               
%               S. Ravishankar, B. E. Moore, R. R. Nadakuditi, and J. A.
%               Fessler, "Efficient online dictionary adaptation and image
%               reconstruction for dynamic MRI,? in Asilomar Conference on
%               Signals, Systems, and Computers, October 2017, to appear.
%               
...
if COMPUTE_STATS
    Dtfit = @(Pt,DBt) 0.5 * norm(vec(Pt - DBt))^2;
    Rtfit = @(D) 0.5 * real(trace(params.Et) + ...
                              trace(D' * (D * params.Gt - 2 * params.Ft)));
    % Cost function (only portion affected by Xt, D, Bt)
    Psi = @(Xt,Pt,D,DBt,Bt) Ytfit(Xt) + ...
                   lambda * (gamma * Rtfit(D) + Dtfit(Pt,DBt) + Btfit(Bt));
    % Cumulative cost function
    CPsi = @(Xt,Pt,D,DBt,Bt) (Psi(Xt,Pt,D,DBt,Bt) + ...
       gamma * (params.yt + lambda * params.bt)) / (1 + gamma * params.ct);
...
for it = 1:nIters
    ...
    itimer = tic;
    if it <= nIters0
        % Fix dictionary for initial iterations
        optsDB.fixedD = true;
    else
    ...
    if COMPUTE_STATS
        ...
        time(it)     = toc(itimer);
        if PRINT_STATS
            out(it,ccost(it),nrmse(it),deltaX(it),deltaD(it), ...
                deltaB(it),sparsity(it),time(it)); 
        end
    end
end
...
if UPDATE_PARAMS
    ...
    params.yt = gamma * params.yt + Ytfit(Xt);
...
function [mu, A, M, xdim, pdim, pgap, type, dr, ddim, unitaryD, fixedD, ...
          nIters, nIters0, nItersDB, nItersX, X0, D0, B0, params, ...
          Xtrue, NRMSEfcn, accel, tau, flag] = parseInputs(Yt,mu,opts)
...
nIters0  = parseField(opts,'nIters0',0);
