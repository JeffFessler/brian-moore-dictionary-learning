...
%                   from time t - 1). Overcomplete (m > p) and
%                   undercomplete (m < p) dictionaries are allowed
%                   
...
% Parse inputs
[D, Bt, params, type, dr, ddim, fixedD, nIters, order, tau, flag] = ...
...
for it = 1:nIters
    ...
    for k = order
        ...
        if OPTIMIZE_DICT
            if ANY_MK
                btMk = btk(Mk)';
                ftk = gamma * params.Ft(:,k) + Yt(:,Mk) * btMk;
                gtk = gamma * params.Gt(:,k) + Bt(:,Mk) * btMk;                
...
% Parse inputs
function [D0, B0, params, type, dr, ddim, fixedD, nIters, order, tau, ...
                                               flag] = parseInputs(Yt,opts)
