function [X, D, B, stats] = dls2(Y,lambda,mu,varargin)
%
% Syntax:       [X, D, B] = dls2(Y,lambda,mu);
%               [X, D, B] = dls2(Y,lambda,mu,opts);
%               [X, D, B, stats] = dls2(Y,lambda,mu);
%               [X, D, B, stats] = dls2(Y,lambda,mu,opts);
%               
...
%                   
%                   opts.accelDB (true) specifies whether to use Nesterov's
%                   acceleration scheme for the (D,B) updates
%                   
%                   opts.tauDB (0.99 + ~accelDB) is the step size
%                   parameter for the (D,B) updates, and should satisfy
%                   
%                       tau <= 1, when accelDB = true
%                       tau <  2, when accelDB = false
%                   
...
% Parse inputs
[A, M, xdim, pdim, pgap, type, dr, ddim, fixedD, nIters, nItersDB, ...
 nItersX, X, D, B, Xtrue, NRMSEfcn, accel, tau, accelDB, tauDB, flag] = ...
...
% Initialize soupDil opts
optsDB.type   = type;
optsDB.dr     = dr;
optsDB.ddim   = ddim;
optsDB.fixedD = fixedD;
optsDB.nIters = nItersDB;
optsDB.accel  = accelDB;
optsDB.tau    = tauDB;
optsDB.flag   = flag - 1;
...
function [A, M, xdim, pdim, pgap, type, dr, ddim, fixedD, nIters, ...
          nItersDB, nItersX, X0, D0, B0, Xtrue, NRMSEfcn, accel, tau, ...
                                accelDB, tauDB, flag] = parseInputs(Y,opts)
...
accelDB  = parseField(opts,'accelDB',true);
tauDB    = parseField(opts,'tauDB',nan);
...
if isnan(tauDB), tauDB = (0.99 + ~accelDB);             end
