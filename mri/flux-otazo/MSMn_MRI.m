function [X NRMSE its rel] = MSMn_MRI(Y,model,opt,GDopt,truth)
%--------------------------------------------------------------------------
% Syntax:       X = MSMn_MRI(Y,model);
%               X = MSMn_MRI(Y,model,opt);
%               X = MSMn_MRI(Y,model,opt,GDopt);
%               X = MSMn_MRI(Y,model,opt,GDopt,truth);
%               [X NRMSE its rel] = MSMn_MRI(Y,model);
%               [X NRMSE its rel] = MSMn_MRI(Y,model,opt);
%               [X NRMSE its rel] = MSMn_MRI(Y,model,opt,GDopt);
%               [X NRMSE its rel] = MSMn_MRI(Y,model,opt,GDopt,truth);
%               
...
%      (svd)          \|P_i L\|_* | 'SVT_LLR'       | {[],nb,lambda}
%                                 |                 | {mask,nb,lambda}
%      (svd)        \|P_i L\|_p^p | 'SVTp_LLR'      | {[],nb,p,lambda}
%                                 |                 | {mask,nb,p,lambda}
%      (svd)     (block-wise RMT) | 'OptShrink_LLR' | {[],nb,r}
%                                 |                 | {mask,nb,r}
%      (svd)  \|P_i(L - Lr)\|_F^2 | 'PCA_LLR'       | {[],nb,r}
%                                 |                 | {mask,nb,r}
%               
...
%        [OPTIONAL] opt.nu      = Restart parameter, in (0,1)       (0.999)
%               
...
%               
%               using Nesterov-accelerated alternating direction method of
%               multipliers (ADMM)
%               
% Author:       Brian Moore
%               brimoor@umich.edu
...
noScaleList = {'optshrink','optshrink_llr','pca','pca_llr','ball2'};
...
Lambda = cell(m,1);
Lambda_hat = cell(m,1);
Lambda_last = cell(m,1);
for i = 1:m
    X{i} = X0;
    Xs{i} = cell(n(i),1);
    Xs_hat{i} = cell(n(i),1);
    Xs_last{i} = cell(n(i),1);
    Lambda{i} = cell(n(i),1);
    Lambda_hat{i} = cell(n(i),1);
    Lambda_last{i} = cell(n(i),1);
    for j = 1:n(i)
        Xs{i}{j} = X0;
        Xs_hat{i}{j} = X0;
        Lambda{i}{j} = 0;
...
while ((its < Nmin) || ((its < Nmax) && (maxRel > epsilon)))
    ...
    % X-updates
    X_last = X; % Save last iterates
    for i = 1:m
        % Gradient function handle
        grad{i} = @(X) mu * At(A(sum(cat(3,X{:}),3)) - Y) + ...
                      rho * (n(i) * X{i} - sum(cat(3,Xs_hat{i}{:}),3) + sum(cat(3,Lambda_hat{i}{:}),3));
    end
    %X = GD(grad,X,tau,k);
    X = nGD(grad,X,tau,k);
    ...
    for i = 1:m
        for j = 1:n(i)
            ...
            if ~any(ismember(noScaleList,str))
                % Scale regularization by ADMM parameter
                params{end} = params{end} / rho;
            end
            
            % Apply proximal operator
            Xs_last{i}{j} = Xs{i}{j}; % Save last iterates
            Xs{i}{j} = Ut(V(prox(Vt(U(X{i} + Lambda_hat{i}{j})),str,params)));
        end
    end
    ...
    for i = 1:m
        for j = 1:n(i)
            Lambda_last{i}{j} = Lambda{i}{j}; % Save last iterates
            Lambda{i}{j} = Lambda_hat{i}{j} + X{i} - Xs{i}{j};
        end
    end
    
    % Check for restart
    t_last = t; % Save last iterate
    c_last = c; % Save last iterate
    c = 0;
    for i = 1:m
        for j = 1:n(i)
            c = c + norm(Lambda{i}{j}(:) - Lambda_hat{i}{j}(:))^2 + ...
                    norm(Xs{i}{j}(:) - Xs_hat{i}{j}(:))^2;
        end
    end
    if (c < (nu * c_last))
        % Nesterov updates
        ...
        for i = 1:m
            for j = 1:n(i)
                Xs_hat{i}{j} = Xs{i}{j} + alpha * (Xs{i}{j} - Xs_last{i}{j});
                Lambda_hat{i}{j} = Lambda{i}{j} + alpha * (Lambda{i}{j} - Lambda_last{i}{j});
            end
        end
    else
        % Restart
        fprintf('*** Restarting... [stability = %.3f] ***\n',c / c_last);
        t = 1;
        c = c_last / nu;
        Xs_hat = Xs_last;
        Lambda_hat = Lambda_last;
    end
    
    % Update relative changes
    rel(its,:) = cellfun(@(Xi,Xli) NRMSEfcn(Xi,Xli),X,X_last);
    maxRel = max(rel(its,:));
