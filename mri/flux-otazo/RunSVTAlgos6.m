function RunSVTAlgos6(idx)
% Syntax:   RunSVTAlgos6(idx);
...
outpath = '/home/brimoor/Research/otazo/svt_LaS_mixed21/data.mat'; % Output data path
%--------------------------------------------------------------------------
...
N = Np * NSNR * Nseed * NlambdaL * NlambdaS;
...
[p_inds SNR_inds seed_inds lambdaL_inds lambdaS_inds] = ind2sub([Np NSNR Nseed NlambdaL NlambdaS],inds);
...
labels = {'p','SNR','seed','lambdaL','lambdaS','NRMSE','NRMSE_ROI1','NRMSE_ROI2'}; %#ok
data = [p(:) SNR(:) seed(:) lambdaL(:) lambdaS(:) nan(Ninds,3)];
for i = 1:Ninds
    % Start simulation timer
    stimer = tic;
    
    % Perform reconstruction
    [~,recon,Xtrue,ROIs] = run_svt_algo(p(i),SNR(i),seed(i),inpath,lambdaL(i),proxS,lambdaS(i),algoStr,T);
    [ny nx] = size(ROIs{1});
    nt = size(Xtrue,2);
