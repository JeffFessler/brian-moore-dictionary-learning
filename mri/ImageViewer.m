function ImageViewer(IST,SLRN,dims)
% load('CardiacPerfComparison.mat');
% ImageViewer(IST,SLRN);
% ImageViewer(IST,SLRN,[33 96 33 96]);
...
% Get sizes (assumed same for IST and SLRN)
[Nparam1 Nparam2] = size(IST.lambdaL);
[nx ny nt] = size(IST.L{1});
if (nargin < 3)
    dims = [1 nx 1 ny];
end
frame = 1;
...
SLRNslider1 = uicontrol('Style','slider','Min',1,'Max',Nparam1,'Value',2,'SliderStep',[1 1] / (Nparam1 - 1),'Units','Normalized','Position',[.35 dh .25 dh],'Callback',@UpdateSLRN);
SLRNslider2 = uicontrol('Style','slider','Min',1,'Max',Nparam2,'Value',Nparam2,'SliderStep',[1 1] / (Nparam2 - 1),'Units','Normalized','Position',[.35 0 .25 dh],'Callback',@UpdateSLRN);
...
function UpdateIST(varargin)
    ...
    set(ISTtext1,'String',sprintf('lambdaL = %.4f',IST.lambdaL(val1,val2)));
    set(ISTtext2,'String',sprintf('lambdaS = %.4f',IST.lambdaS(val1,val2)));
...
function UpdateSLRN(varargin)
    ...
    % Update slider labels
    set(SLRNtext1,'String',sprintf('r = %i',SLRN.r(val1,val2)));
    set(SLRNtext2,'String',sprintf('lambdaS = %.4f',SLRN.lambdaS(val1,val2)));
