function [Xtrue Xfft mask ROIs nd splineInterp Y A] = GenerateFesslerPhantom(nc,nt,SNR,seed,dyn_obj,dce)
% Syntax:   [Xtrue Xfft mask ROIs nd splineInterp Y A] = GenerateFesslerPhantom(nc,nt,SNR,seed,dyn_obj,dce);
%           
%               Xtrue = nx x ny x nt
%               Xfft = nx x ny x nt
%               mask = nx x ny
...
% Generate system matrix, if necessary
if (nargout >= 8)
    % Coil maps
    tmp = cell(nc,1);
    for ic = 1:nc
