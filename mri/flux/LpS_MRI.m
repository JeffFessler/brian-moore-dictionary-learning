function [L S NRMSE its rel] = LpS_MRI(Y,model,opt,truth)
%--------------------------------------------------------------------------
% Syntax:       [L S] = LpS_MRI(Y,model);
%               [L S] = LpS_MRI(Y,model,opt);
%               [L S] = LpS_MRI(Y,model,opt,truth);
%               [L S NRMSE its rel] = LpS_MRI(Y,model);
%               [L S NRMSE its rel] = LpS_MRI(Y,model,opt);
%               [L S NRMSE its rel] = LpS_MRI(Y,model,opt,truth);
%               
...
%               
% {L,S} = argmin 0.5 ||Y - A(L + S)||^2 + lambdaL ||L||* + lambdaS ||TS||1
%               
    ...
    switch lower(opt.methodL)
        ...
        case 'pca'
            if (useLLR == false)
                ...
                MmS = embed(M - S,model.mask);
                L = masker(PCA_LLR(MmS,model.r,model.nb),model.mask);
            end
        otherwise
